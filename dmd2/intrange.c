...
#if WANT_CENT
        rvVal = UINT128C(0ULL, 1ULL) << 127;
#else
        rvVal = 1ULL << 63;
#endif
        aAbs >>= 1;
#if WANT_CENT
        if (aAbs & UINT128C(0xAAAAAAAAAAAAAAAAULL, 0xAAAAAAAAAAAAAAAAULL)) rvVal >>= 1;
        if (aAbs & UINT128C(0xCCCCCCCCCCCCCCCCULL, 0xCCCCCCCCCCCCCCCCULL)) rvVal >>= 2;
        if (aAbs & UINT128C(0xF0F0F0F0F0F0F0F0ULL, 0xF0F0F0F0F0F0F0F0ULL)) rvVal >>= 4;
        if (aAbs & UINT128C(0xFF00FF00FF00FF00ULL, 0xFF00FF00FF00FF00ULL)) rvVal >>= 8;
        if (aAbs & UINT128C(0xFFFF0000FFFF0000ULL, 0xFFFF0000FFFF0000ULL)) rvVal >>= 16;
        if (aAbs & UINT128C(0xFFFFFFFF00000000ULL, 0xFFFFFFFF00000000ULL)) rvVal >>= 32;
        if (aAbs & UINT128C(0xFFFFFFFFFFFFFFFFULL, 0x0000000000000000ULL)) rvVal >>= 64;
#else
