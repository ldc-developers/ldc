...
llvm::GlobalVariable *IrAggr::getInterfaceVtbl(BaseClass *b, bool new_instance,
  ...
  for (size_t i = b->sym->vtblOffset(); i < n; i++) {
    ...
    if (thunkOffset == 0) {
      constants.push_back(DtoBitCast(irFunc->getCallee(), voidPtrTy));
      continue;
    }
    ...
    if (!thunk) {
      ...
      thunk = LLFunction::Create(
          isaFunction(irFunc->getCallee()->getType()->getContainedType(0)), lwc.first,
          thunkName, &gIR->module);
      setLinkage(lwc, thunk);
      thunk->copyAttributesFrom(irFunc->getCallee());
...
// function has.
      ...
      llvm::Function::arg_iterator origArg = irFunc->getCallee()->arg_begin();
      ...
      llvm::CallInst *call = gIR->ir->CreateCall(irFunc->getCallee(), args);
