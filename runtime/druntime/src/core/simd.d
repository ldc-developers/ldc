...
version (LDC)
{
static if (is(Vector!(void[4])))    alias void4 = Vector!(void[4]);        ///
static if (is(Vector!(byte[4])))    alias byte4 = Vector!(byte[4]);        ///
static if (is(Vector!(ubyte[4])))   alias ubyte4 = Vector!(ubyte[4]);       ///
static if (is(Vector!(short[2])))   alias short2 = Vector!(short[2]);       ///
static if (is(Vector!(ushort[2])))  alias ushort2 = Vector!(ushort[2]);      ///
}
