...
private:
  ...
  void addTrivialMapRecursively(Type *Ty);
};
}
...
bool TypeMapTy::areTypesIsomorphic(Type *DstTy, Type *SrcTy) {
  ...
  // non-speculatively.
  if (DstTy == SrcTy) {
    addTrivialMapRecursively(DstTy);
    return true;
  }
...
/// For types which turn out to be the same Type instance in both modules,
/// remembers this fact for it and all the contained types.
///
/// We need to be aware of this when performing the instruction rewrites
/// laster on.
void TypeMapTy::addTrivialMapRecursively(Type *Ty) {
  ...
  for (unsigned I = 0, E = Ty->getNumContainedTypes(); I != E; ++I) {
    addTrivialMapRecursively(Ty->getContainedType(I));
  }
}
