...
public:
  explicit DComputeCodegenVisitor(IRState *irs, DComputeTarget &_dct)
      : irs(irs), dct(_dct) {}
  ...
  void visit(Nspace *ns) LLVM_OVERRIDE {
    IF_LOG Logger::println("@compute Nspace::codegen for %s",
                           ns->toPrettyChars());
    LOG_SCOPE
  ...
  void visit(TypeInfoDeclaration *decl) LLVM_OVERRIDE {
    ...
    IF_LOG Logger::println(
        "@compute ignoring TypeInfoDeclaration::codegen: '%s'",
        decl->toPrettyChars());
  }

  void visit(TypeInfoClassDeclaration *decl) LLVM_OVERRIDE {
    IF_LOG Logger::println("@compute ignoring TypeInfoDeclaration::codegen: "
