...
struct DComputeSemanticAnalyser : public StoppableVisitor {
  ...
  void visit(AssertExp *e) override {
    // Don't descend into the string generated by -checkaction=context
    if (!e->msg)
      return;
    CallExp * ce = e->msg->isCallExp();
    if (ce && ce->f->ident == Id::_d_assert_fail)
      stop = true;
  }
  ...
  void visit(FuncDeclaration *fd) override {
    ...
    // Dont descend into _d_assert_fail which is generated by -checkaction=context
    if (fd->ident = Id::_d_assert_fail)
      stop = true;
    currentFunction = fd;
  }
