...
DISubroutineType DIBuilder::CreateFunctionType(Type *type,
  ...
  if (fd) {
    ...
    if (auto parentAggregate = fd->isThis()) {
      ...
      pointeeType = Type::tuns8; // cannot use void
    }

    if (pointeeType) {
      // TODO: extra flags (DIFlagArtificial | DIFlagObjectPointer) like clang?
      auto ditype = DBuilder.createReferenceType(
          llvm::dwarf::DW_TAG_pointer_type, CreateTypeDescription(pointeeType),
          target.ptrsize * 8);
      params.emplace_back(ditype);
    }
  }
