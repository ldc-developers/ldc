...
private:
  ...
  bool requireHardfloatRewrite(Type *ty) {
    ...
    if (result.length <= 0)
      return false;
    if (result.length == 1)
      return isFloating(result.fields[0]);
    return isFloating(result.fields[0]) || isFloating(result.fields[1]);
  }
...
public:
  ...
  void rewriteArgument(IrFuncTy &fty, IrFuncTyArg &arg, bool isVararg) {
    ...
    if (isIntegral(ty) && (ty->ty == TY::Tint32 || ty->ty == TY::Tuns32 ||
                           ty->ty == TY::Tdchar)) {
