...
public:
  ...
  enum FloatSemanticsInD { Float, Double, LongDouble, NumModes };
...
private:
  ...
  longdouble &init(const longdouble &other) { return init(*other.value()); }
...
public:
  ...
  longdouble operator+(const longdouble &r) {
    longdouble tmp;
    tmp.init(*this).value()->add(*r.value(),
  ...
  longdouble operator-(const longdouble &r) {
    longdouble tmp;
    tmp.init(*this).value()->subtract(*r.value(),
                                      llvm::APFloat::rmNearestTiesToEven);
    return tmp;
  }
  ...
  longdouble operator*(const longdouble &r) {
    longdouble tmp;
    tmp.init(*this).value()->multiply(*r.value(),
                                      llvm::APFloat::rmNearestTiesToEven);
    return tmp;
  }
  ...
  longdouble operator/(const longdouble &r) {
    longdouble tmp;
    tmp.init(*this).value()->divide(*r.value(),
                                    llvm::APFloat::rmNearestTiesToEven);
    return tmp;
  }
  ...
  longdouble operator%(const longdouble &r) {
    longdouble tmp;
    tmp.init(*this).value()->mod(*r.value(),
                                 llvm::APFloat::rmNearestTiesToEven);
    return tmp;
  }
  ...
  operator float() const { return convertToFloat(); }
  operator double() const { return convertToDouble(); }

  operator signed char() const {
    return convertToInteger<signed char, false>();
  }
  ...
  operator unsigned char() const {
    return convertToInteger<unsigned char, true>();
  }
  operator unsigned short() const {
    return convertToInteger<unsigned short, true>();
  }
  operator unsigned int() const {
    return convertToInteger<unsigned int, true>();
  }
  operator unsigned long() const {
    return convertToInteger<unsigned long, true>();
  }
  operator unsigned long long() const {
    return convertToInteger<unsigned long long, true>();
  }

  operator llvm::APFloat &() { return *value(); }

  operator const llvm::APFloat &() const { return *value(); }

  operator bool() const { return value()->isZero() ? false : true; }

  double convertToDouble() const {
    llvm::APFloat trunc(*value());
    bool ignored;
    llvm::APFloat::opStatus status =
        trunc.convert(llvm::APFloat::IEEEdouble,
  ...
  float convertToFloat() const {
    ...
    bool ignored;
    llvm::APFloat::opStatus status =
        trunc.convert(llvm::APFloat::IEEEsingle,
                      llvm::APFloat::rmNearestTiesToEven, &ignored);
    // assert(status == llvm::APFloat::opOK);
    (void)status;
  ...
  template <typename T, bool IsUnsigned> T convertToInteger() const {
    llvm::APSInt val(8 * sizeof(T), IsUnsigned);
    bool ignored;
    value()->convertToInteger(val, llvm::APFloat::rmNearestTiesToEven,
                              &ignored);
  ...
  longdouble &set(int8_t i) {
    ...
    v.convertFromSignExtendedInteger(&tmp, 1, true,
                                     llvm::APFloat::rmNearestTiesToEven);
    return init(v);
  }
  ...
  longdouble &set(uint8_t i) {
    const llvm::integerPart tmp = static_cast<llvm::integerPart>(i);
    llvm::APFloat v(getFltSemantics(), llvm::APFloat::uninitialized);
    v.convertFromSignExtendedInteger(&tmp, 1, false,
                                     llvm::APFloat::rmNearestTiesToEven);
    return init(v);
  }
...
// are also needed to play with mars.h typedefs (dinteger_t and sinteger_t).
  ...
  longdouble &set(float r) {
    ...
    bool ignored;
    llvm::APFloat::opStatus status = extended.convert(
        getFltSemantics(), llvm::APFloat::rmNearestTiesToEven, &ignored);
    assert(status == llvm::APFloat::opOK);
    (void)status;
  ...
  longdouble &set(const llvm::APFloat &r) { return init(r); }

  longdouble &set(const longdouble &r) { return init(r); }
  ...
  static longdouble convertFromString(const char *str) {
    llvm::APFloat v(getFltSemantics(), llvm::APFloat::uninitialized);
    v.convertFromString(llvm::StringRef(str),
                        llvm::APFloat::rmNearestTiesToEven);
    longdouble tmp;
    return tmp.init(v);
  }
...
template <typename T> inline longdouble ldouble(T x) {
  longdouble d;
  d.set(x);
  return d;
}

template <typename T> inline longdouble operator+(longdouble ld, T x) {
  return ld + ldouble(x);
}
template <typename T> inline longdouble operator-(longdouble ld, T x) {
  return ld - ldouble(x);
...
template <typename T> inline longdouble operator+(T x, longdouble ld) {
  return ldouble(x) + ld;
}
template <typename T> inline longdouble operator-(T x, longdouble ld) {
  return ldouble(x) - ld;
...
template <typename T> inline bool operator<(longdouble ld, T x) {
  return ld < ldouble(x);
}
template <typename T> inline bool operator<=(longdouble ld, T x) {
  return ld <= ldouble(x);
...
template <typename T> inline bool operator<(T x, longdouble ld) {
  return ldouble(x) < ld;
}
template <typename T> inline bool operator<=(T x, longdouble ld) {
  return ldouble(x) <= ld;
...
inline longdouble fminl(longdouble x, longdouble y) {
  return ldc::longdouble::fmin(x, y);
}
inline longdouble fmaxl(longdouble x, longdouble y) {
  return ldc::longdouble::fmax(x, y);
}
inline longdouble fmodl(longdouble x, longdouble y) {
  return ldc::longdouble::fmod(x, y);
}
inline longdouble ldexpl(longdouble ldval, int exp) {
  return ldc::longdouble::ldexp(ldval, exp);
}
...
// Macros are used by the D frontend, so map to longdouble property values
// instead of host long double.
#undef FLT_MAX
#undef DBL_MAX
#undef LDBL_MAX
#undef FLT_MIN
...
#define LDBL_MAX_10_EXP                                                        \
  ldc::real_limits[ldc::longdouble::LongDouble].max_10_exp;
...
#define LDBL_MIN_10_EXP                                                        \
  ldc::real_limits[ldc::longdouble::LongDouble].min_10_exp;
