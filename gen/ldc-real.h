//===-- gen/ldc-real.h - Interface of longdouble for LDC ------------*- C++ -*-===//
//
...
private:
    // Points to the member below if object is constructed
    llvm::APFloat *value;
    ...
    {
        if (static_cast<void *>(this->value) == static_cast<void *>(&this->mem))
            this->value->~APFloat();
        this->value = new (&this->mem) llvm::APFloat(other);
        return *this;
    }
...
public:
    ...
    {
        longdouble tmp;
        tmp.init(*this).value->add(*r.value, llvm::APFloat::rmTowardZero);
        return tmp;
    }
    ...
    {
        longdouble tmp;
        tmp.init(*this).value->subtract(*r.value, llvm::APFloat::rmTowardZero);
        return tmp;
    }
    ...
    {
        longdouble tmp;
        tmp.init(*this).value->multiply(*r.value, llvm::APFloat::rmTowardZero);
        return tmp;
    }
    ...
    {
        longdouble tmp;
        tmp.init(*this).value->divide(*r.value, llvm::APFloat::rmTowardZero);
        return tmp;
    }
    ...
    {
        longdouble tmp;
        tmp.init(*this).value->mod(*r.value, llvm::APFloat::rmTowardZero);
        return tmp;
    }
    ...
    longdouble &set(int8_t i)
    {
        ...
        return init(llvm::APFloat(getFltSemantics(), tmp));
    }
