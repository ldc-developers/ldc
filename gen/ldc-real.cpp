...
const llvm::fltSemantics &ldc::longdouble::getFltSemantics() {
  ...
  case llvm::Triple::x86_64:
    return longdouble64 ? llvm::APFloat::IEEEdouble
...
static llvm::APFloat::opStatus sin(llvm::APFloat &the) {
  ...
  if (negate)
    the.changeSign();
  llvm::APFloat the2(the);
  the2.multiply(the2, llvm::APFloat::rmNearestTiesToEven);
...
ldc::longdouble ldc::longdouble::log() const {
  //  warning(Loc(), "log() uses only double precision");
  double d = std::log(value()->convertToDouble());
  return ldouble(d);
}
...
ldc::longdouble ldc::longdouble::fmin(longdouble x, longdouble y) {
  ...
  return (x.isNaN() ? y : y.isNaN() ? x : x <= y ? x : y);
}
...
int ldc::longdouble::formatHex(char *buf, bool upper) const {
  return value()->convertToHexString(buf, 0, upper,
...
static llvm::APFloat toLD(const llvm::APFloat &f) {
  ...
  llvm::APFloat::opStatus status =
      cvt.convert(ldc::longdouble::getFltSemantics(),
...
void ldc::real_init() {
  ...
  case 53:
    ...
    break;
  default:
    llvm_unreachable("Floating point type not supported");
  }

  real_limits[longdouble::Float].maxval =
      toLD(APFloat::getLargest(APFloat::IEEEsingle));
  real_limits[longdouble::Double].maxval =
      toLD(APFloat::getLargest(APFloat::IEEEdouble));
  real_limits[longdouble::LongDouble].maxval = APFloat::getLargest(ldSem);

  real_limits[longdouble::Float].minval =
      toLD(APFloat::getSmallestNormalized(APFloat::IEEEsingle));
  real_limits[longdouble::Double].minval =
      toLD(APFloat::getSmallestNormalized(APFloat::IEEEdouble));
  real_limits[longdouble::LongDouble].minval =
      APFloat::getSmallestNormalized(ldSem);

  real_limits[longdouble::Float].epsilonval = APFloat(ldSem, "1.19209290e-07");
  real_limits[longdouble::Double].epsilonval =
      APFloat(ldSem, "2.2204460492503131e-016");
  switch (real) {
  case x87:
    real_limits[longdouble::LongDouble].epsilonval =
        APFloat(ldSem, "1.0842021724855044340e-019");
    break;
  case ppc:
    real_limits[longdouble::LongDouble].epsilonval =
        APFloat(ldSem, "2.2204460492503131e-016");
  ...
  switch (real) {
  case x87:
    real_limits[longdouble::LongDouble].dig = 80;
    break;
  case ppc:
    real_limits[longdouble::LongDouble].dig = 15;
    break; // like double
  case other:
    real_limits[longdouble::LongDouble].dig = 15;
    break; // aka double
  ...
  real_limits[longdouble::Float].mant_dig =
      APFloat::semanticsPrecision(APFloat::IEEEsingle);
  real_limits[longdouble::Double].mant_dig =
      APFloat::semanticsPrecision(APFloat::IEEEdouble);
  real_limits[longdouble::LongDouble].mant_dig =
      APFloat::semanticsPrecision(ldSem);

  real_limits[longdouble::Float].max_10_exp = 38;
  real_limits[longdouble::Double].max_10_exp = 308;
  switch (real) {
  case x87:
    real_limits[longdouble::LongDouble].max_10_exp = 4932;
    break;
  case ppc:
    real_limits[longdouble::LongDouble].max_10_exp = 308;
    break; // like double
  case other:
    real_limits[longdouble::LongDouble].max_10_exp = 308;
    break; // aka double
  ...
  switch (real) {
  case x87:
    real_limits[longdouble::LongDouble].min_10_exp = -4932;
    break;
  case ppc:
    real_limits[longdouble::LongDouble].min_10_exp = -307;
    break; // like double
  case other:
    real_limits[longdouble::LongDouble].min_10_exp = -307;
    break; // aka double
  ...
  switch (real) {
  case x87:
    real_limits[longdouble::LongDouble].max_exp = 16384;
    break;
  case ppc:
    real_limits[longdouble::LongDouble].max_exp = 1024;
    break; // like double
  case other:
    real_limits[longdouble::LongDouble].max_exp = 1024;
    break; // aka double
  ...
  switch (real) {
  case x87:
    real_limits[longdouble::LongDouble].min_exp = -16381;
    break;
  case ppc:
    real_limits[longdouble::LongDouble].min_exp = -1021;
    break; // like double
  case other:
    real_limits[longdouble::LongDouble].min_exp = -1021;
    break; // aka double
